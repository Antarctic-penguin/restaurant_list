      ...
      restaurantType
        ...
        .then(restaurantType => {
          ...
          if (restaurants.length === 0) {
            res.render('notfound', { keyword, type, restaurantCategory, sortRoute })
          } else {
            res.render('index', { sort, restaurants, keyword, type, restaurantCategory, sortRoute, typeRoute, keywordRoute })
          }
        })
        .catch(error => console.error(error))
    })
    ...
    .then(()=>{
      ...
      restaurantType
        ...
        .then(restaurantType => {
          ...
          if (restaurants.length === 0) {
            res.render('notfound', { sortRoute, keyword, type, restaurantCategory })
          } else {
            res.render('index', { sort, sortRoute, keyword, restaurants, type, restaurantCategory, typeRoute, keywordRoute })
          }
        })
        .catch(error => console.error(error))
    })
...
router.get('/new', (req, res) => {
  getCategory()
    ...
    .then(()=>{
      res.render('new', { restaurantCategory})
    })
    .catch(error => console.error(error))
})
// 新增餐廳路由
